        ...
        <TransitionScale v-drag-scroll appear group tag="div" class="slider flex items-start overflow-x-auto pb-1 scrollbar-none gap" :class="overflow ? 'overflow px-margin -mx-margin' : 'rounded'">
            ...
            <div v-for="(show, index) in shows" :key="index" class="group relative cursor-pointer rounded">
                <div class="relative mb-3 aspect-2/3 overflow-hidden card w-media-card">
                    <img v-if="show.poster_path" :src="$getImageUrl(show.poster_path, 'poster', 'w342')" loading="lazy" :alt="show.name" class="h-full w-full object-cover transition-all duration-300 group-hover:scale-102 group-hover:blur-sm" />
