    ...
    actions: {
        async login(username: string, password: string) {
            ...
            try {
                ...
                const sessionResponse: CreateSession = await $fetch(
                    '/api/authentication/session/new',
                    {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
        ...
        async createRequestToken() {
            try {
                return await $fetch<CreateRequestToken>(
                    '/api/authentication/token/new',
                );
            } catch (error) {
                console.error(error);
        ...
        async fetchRatedMovies() {
            const sessionId = this.sessionId;
            if (!sessionId || !this.accountDetails?.id) return;
            this.ratedMovies = await $fetch<RatedMovieResponse>(
                `/api/account/${this.accountDetails.id}/rated/movies`,
                {
                    headers: { 'x-tmdb-session-id': sessionId },
                },
            );
        ...
        async fetchRatedTVShows() {
            const sessionId = this.sessionId;
            if (!sessionId || !this.accountDetails?.id) return;
            this.ratedTVShows = await $fetch<RatedTvShowResponse>(
                `/api/account/${this.accountDetails.id}/rated/tv`,
                {
                    headers: { 'x-tmdb-session-id': sessionId },
                },
            );
