    ...
    <div class="home-page">
        <div class="grid grid-cols-7 mb-15 gap">
            ...
            <div v-if="!isLoggedIn" class="relative col-span-1 flex flex-shrink-0 flex-col cursor-pointer items-center justify-center gap-2.5 card transition-colors duration-300 focus:bg-primary/50 hover:bg-primary/50">
                <span class="i-ph-plus-square size-6" />
                <h3>more</h3>
            </div>
        </div>
        <MovieSlider v-if="trendingMovies?.results" :after="{ text: 'All movies', link: '/movie', icon: 'i-ph-film-strip-bold' }" title="Trending movies" :overflow="true" :movies="trendingMovies.results" class="mb-10" />
        <ShowSlider v-if="trendingShows?.results" :after="{ text: 'All TV shows', link: '/tv', icon: 'i-ph-television-simple-bold' }" title="Trending TV shows" :overflow="true" :shows="trendingShows.results" class="mb-10" />
